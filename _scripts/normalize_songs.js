...
function normalizeFile(filePath) {
  ...
  if(!fm.title&&!body) return {fn:fn,ch:['  SKIPPED'],out:raw,profiles:[]};
  ...
  return {fn:fn,ch:ch,out:writeFrontmatter(fm)+'\n\n'+body+'\n',profiles:fm.profile};
}
...
for(var fi=0;fi<files.length;fi++){
  try{
    ...
    if(apply) fs.writeFileSync(path.join(outputDir,files[fi]),r.out,'utf8');
