...
/**
 * add-feel-to-songs-v1.js
 *
 * Purpose:
 * - Add `feel: []` to migrated song frontmatter.
 *
 * Rules:
 * - Insert `feel: []` immediately after `tempo:`
 * - Preserve all other fields and body content
 * - If `feel` already exists, leave the file unchanged
 * - Dry run by default
 * - In apply mode, write to a new output folder unless overridden
 *
 * Usage:
 *   node add-feel-to-songs-v1.js /path/to/songs_migrated_v1
 *   node add-feel-to-songs-v1.js /path/to/songs_migrated_v1 --apply
 *   node add-feel-to-songs-v1.js /path/to/songs_migrated_v1 --apply --out /path/to/output
 */
...
const outputDir =
  outIndex >= 0 && process.argv[outIndex + 1]
    ...
    : path.join(process.cwd(), "songs_with_feel_v1");

if (!inputDir) {
  console.error(
    "Usage: node add-feel-to-songs-v1.js /path/to/songs_migrated_v1 [--apply] [--out /path/to/output]"
  );
  process.exit(1);
}
...
function migrateFile(fullPath) {
  ...
  if (Object.prototype.hasOwnProperty.call(oldData, "feel")) {
    changes.push("feel already present");
  } else {
    changes.push("added feel as empty array");
  }
...
const reportPath = APPLY
  ? path.join(outputDir, "_add_feel_report.txt")
  : path.join(process.cwd(), "_add_feel_report_dryrun.txt");
