...
/**
 * add-song-category-fields-v1.js
 *
 * Purpose:
 * - Add `style: []`, `tradition: []`, and `use: []` to song frontmatter.
 *
 * Rules:
 * - Insert after `function: []`
 * - Preserve all other fields and body content
 * - If fields already exist, preserve them
 * - Dry run by default
 * - In apply mode, write to a new output folder unless overridden
 *
 * Usage:
 *   node add-song-category-fields-v1.js /path/to/songs
 *   node add-song-category-fields-v1.js /path/to/songs --apply
 *   node add-song-category-fields-v1.js /path/to/songs --apply --out /path/to/output
 */
...
const outputDir =
  outIndex >= 0 && process.argv[outIndex + 1]
    ...
    : path.join(process.cwd(), "songs_with_category_fields_v1");

if (!inputDir) {
  console.error(
    "Usage: node add-song-category-fields-v1.js /path/to/songs [--apply] [--out /path/to/output]"
  );
  process.exit(1);
}
...
function migrateFile(fullPath) {
  ...
  if (!Object.prototype.hasOwnProperty.call(oldData, "style")) {
    changes.push("added style as empty array");
  }
  if (!Object.prototype.hasOwnProperty.call(oldData, "tradition")) {
    changes.push("added tradition as empty array");
  }
  if (!Object.prototype.hasOwnProperty.call(oldData, "use")) {
    changes.push("added use as empty array");
  }

  if (changes.length === 0) {
    changes.push("category fields already present");
  }
...
const reportPath = APPLY
  ? path.join(outputDir, "_add_song_category_fields_report.txt")
  : path.join(process.cwd(), "_add_song_category_fields_report_dryrun.txt");
