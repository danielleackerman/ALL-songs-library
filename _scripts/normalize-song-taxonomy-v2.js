 ...
 * - No blind hyphen splitting of profile tokens
 * - Better handling of legacy hyphenated forms
 * - Better handling of compact legacy profile codes
 *
 * Usage:
 *   node normalize-song-taxonomy-v2.js ../songs_with_category_fields_v2
 *   node normalize-song-taxonomy-v2.js ../songs_with_category_fields_v2 --apply --out ../songs_taxonomy_normalized_v2
 */
...
const outputDir =
  outIndex >= 0 && process.argv[outIndex + 1]
    ...
    : path.join(process.cwd(), "songs_taxonomy_normalized_v2");

if (!inputDir) {
  console.error(
    "Usage: node normalize-song-taxonomy-v2.js /path/to/input [--apply] [--out /path/to/output]"
  );
  process.exit(1);
}
...
const EXACT_MAPPINGS = {
  ...
  "medium-slow": [{ field: "tempo", value: "medium-slow" }],
  "med-slow": [{ field: "tempo", value: "medium-slow" }],
  "medium": [{ field: "tempo", value: "medium" }],
  "medium fast": [{ field: "tempo", value: "medium-fast" }],
  "medium-fast": [{ field: "tempo", value: "medium-fast" }],
  "med-fast": [{ field: "tempo", value: "medium-fast" }],
  "(medium) fast": [{ field: "tempo", value: "fast" }],
  "fast": [{ field: "tempo", value: "fast" }],
  "contemp-med": [
    { field: "tradition", value: "contemporary" },
    { field: "tempo", value: "medium" }
  ],
  "maranatha-medium": [
    { field: "style", value: "maranatha" },
    { field: "tempo", value: "medium" }
  ],
  ...
  "ccm-gospel": [{ field: "style", value: "ccm-gospel" }],
  "gospel-ccli": [{ field: "style", value: "ccm-gospel" }],
  ...
  "black-gospel-opera": [{ field: "style", value: "black-gospel-opera" }],
  "contemp-christian": [{ field: "style", value: "ccm" }],
  "jazz": [],
  "jazzy": [],
  "swing-ballad": [],
  "bluesy": [],
...
function generateProfileCandidates(profileArr) {
  const raw = ensureArray(profileArr)
    .join(" ")
    .replace(/[,_]+/g, " ")
    .replace(/[|]+/g, " ")
    .replace(/[;]+/g, " ")
  ...
  const normalizedWhole = normalizeToken(raw);

  const splitOnSpacesOnly = raw
    .split(/\s+/)
    .map(normalizeToken)
    .filter(Boolean);

  const splitOnCommonSeparators = raw
    .split(/[|;,]/)
    .map(normalizeToken)
    .filter(Boolean);
  ...
  const candidates = unique([
    normalizedWhole,
    ...splitOnCommonSeparators,
    ...splitOnSpacesOnly
  ]);

  // Sort longest first so exact compound phrases win.
...
function normalizeSong(fullPath) {
  ...
  const incomingProfileCandidates = generateProfileCandidates(oldData.profile);
  ...
  // Process profile candidates carefully
  for (const token of incomingProfileCandidates) {
...
const mainReportPath = APPLY
  ? path.join(outputDir, "_normalize_song_taxonomy_v2_report.txt")
  : path.join(process.cwd(), "_normalize_song_taxonomy_v2_report_dryrun.txt");

const leftoversPath = APPLY
  ? path.join(outputDir, "_normalize_song_taxonomy_v2_leftovers.txt")
  : path.join(process.cwd(), "_normalize_song_taxonomy_v2_leftovers_dryrun.txt");
